...
module.exports.createProfile = async (values) => {
    ...
    let sql = "CREATE TABLE IF NOT EXISTS profile (id INT AUTO_INCREMENT PRIMARY KEY, email VARCHAR(255), phone VARCHAR(255), address VARCHAR(255), firstName VARCHAR(255), lastName VARCHAR(255), gender VARCHAR(255), profileImage VARCHAR(255),registrationID int ,FOREIGN KEY (registrationID) REFERENCES user(id))";
    await query(sql).then(response => {
        let sql = "INSERT INTO profile (email, phone, address, firstName,lastName,gender,profileImage,registrationID) VALUES ?";
...
module.exports.addAddress = async (req, res) => {
    ...
    let sql = "CREATE TABLE IF NOT EXISTS address (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), phone VARCHAR(255), address VARCHAR(255), city VARCHAR(255), zip VARCHAR(255), type VARCHAR(255),profileID int ,FOREIGN KEY (profileID) REFERENCES profile(id),userID int ,FOREIGN KEY (userID) REFERENCES user(id))";
    await query(sql).then(response => {
